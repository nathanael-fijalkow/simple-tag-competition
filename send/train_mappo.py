...
# =========================
DEVICE = "cpu"          # set "cuda" if you have it
NUM_ENVS = 8
ROLLOUT_T = 256
UPDATES = 2000          # increase (4000+) for stronger results
MAX_CYCLES = 200        # longer horizon than test_agent.py
...
# =========================
# Running mean/std (obs/state normalization)
# =========================
...
# =========================
# Optional: load prey model (same as eval) if available
# =========================
...
def main():
    ...
    # Create envs and reset once to detect obs dims
    ...
    for update in range(1, UPDATES + 1):
        ...
        # Update RMS from current snapshot
        sample_obs = []
        sample_state = []
        ...
        # rollout buffers (ALL DETACHED TENSORS)
        ...
        for t in range(ROLLOUT_T):
            ...
            for e in range(NUM_ENVS):
                ...
                # team reward (mean predator reward)
                r = 0.0
                for aid in ADV_IDS:
        ...
        for _ in range(EPOCHS):
            for start in range(0, B, MINIBATCH):
                ...
                # ---------- Critic update (fresh forward pass!) ----------
                v = critic(state[mb])
